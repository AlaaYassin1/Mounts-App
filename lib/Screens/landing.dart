import 'package:flutter/material.dart';
import 'package:flutter_application_1/main.dart';
import 'package:flutter_application_1/models/data_models.dart';

final Color mainColor = Colors.red[400]!;

//الكونتينر يميل للتوسع لو م اعطيته ابن يمد ياخد كل شي لكن لو اعطيته ابن ياخد ع قد ابنه
class MountsApp extends StatefulWidget {
  const MountsApp({super.key});

  @override
  State<MountsApp> createState() => _MountsAppState();
}

class _MountsAppState extends State<MountsApp> {
  int currentindex = 0;
  List<Widget> pages = [
    HomePage(),
    Text("Explore"),
    Text("Bookmark"),
    Text('prodile')
  ];
  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        backgroundColor: Colors.white,
        appBar: AppBar(
          backgroundColor: Colors.transparent,
          elevation: 0,
          iconTheme: IconThemeData(
            color: mainColor,
          ),
          actions: [
            SizedBox(
              width: 40,
            )
          ],
          title: Center(
            child: Icon(
              Icons.terrain,
              size: 40,
            ),
          ),
        ),
        drawer: Drawer(
          child: Container(
            padding: EdgeInsets.only(left: 30, bottom: 20),
            alignment: Alignment.bottomLeft,
            color: mainColor,
            child: Icon(
              Icons.terrain,
              color: Colors.white,
              size: 60,
            ),
          ),
        ),
        bottomNavigationBar: BottomNavigationBar(
            currentIndex: currentindex,
            onTap: (value) {
              setState(() {
                currentindex = value;
              });
              //  print("$value");
            },
            unselectedItemColor: Colors.grey,
            showSelectedLabels: false,
            items: [
              BottomNavigationBarItem(
                  icon: Icon(
                    Icons.home,
                  ),
                  activeIcon: ActiveNavItem(
                    icon: Icons.home,
                    label: 'Home',
                  ),
                  label: ''),
              BottomNavigationBarItem(
                  icon: Icon(
                    Icons.explore,
                  ),
                  activeIcon: ActiveNavItem(
                    icon: Icons.explore,
                    label: 'Explore',
                  ),
                  label: ''),
              BottomNavigationBarItem(
                  icon: Icon(
                    Icons.bookmark_border,
                  ),
                  activeIcon: ActiveNavItem(
                    icon: Icons.bookmark_border,
                    label: 'Bookmark',
                  ),
                  label: ''),
              BottomNavigationBarItem(
                  icon: Icon(
                    Icons.person,
                  ),
                  activeIcon: ActiveNavItem(
                    icon: Icons.person,
                    label: 'Profile',
                  ),
                  label: ''),
            ]),
        body: pages[currentindex], //HomePage(),
      ),
    );
  }
}

class HomePage extends StatelessWidget {
  const HomePage({
    Key? key,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        AppHeader(),
        AppSearch(),
        Expanded(
            child:
                Listviewbuilder()), //صلاحيات ياخد وسع //ياخد الي بظل من وسع بديش بالشاشة ولا نتفة بيضة
        AppCategoryList(),
      ],
    );
  }
}

class ActiveNavItem extends StatelessWidget {
  final IconData icon;
  final String label;
  final bool isActive;
  const ActiveNavItem(
      {super.key,
      required this.icon,
      required this.label,
      this.isActive = true});

  @override
  Widget build(BuildContext context) {
    return isActive
        ? Container(
            // alignment: Alignment.center,
            //  margin: EdgeInsets.symmetric(horizontal: 20),
            padding: const EdgeInsets.only(
              left: 15,
              top: 5,
              bottom: 5,
              right: 15,
            ),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(20),
              color: mainColor,
            ),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              // mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(icon, color: Colors.white),
                const SizedBox(width: 5),
                Text(
                  label,
                  style: const TextStyle(
                    color: Colors.white,
                  ),
                ),
              ],
            ),
          )
        : Icon(icon, color: Colors.grey);
  }
}

class AppHeader extends StatelessWidget {
  const AppHeader({super.key});

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(top: 30, right: 30, left: 30, bottom: 30),
      child: Container(
        child: Row(
          children: [
            ClipOval(
              child: Image.network(
                'data:image/jpeg;base64,',
                fit: BoxFit.cover,
                width: 70,
                height: 70,
              ),
            ),
            SizedBox(width: 30),
            Column(
              children: [
                Text(
                  'Hello,Roman',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 20,
                  ),
                ),
                Text(
                  'Good Morning',
                  style: TextStyle(color: mainColor),
                )
              ],
            ),
          ],
        ),
      ),
    );
  }
}

class AppSearch extends StatelessWidget {
  const AppSearch({super.key});

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 30, right: 30, left: 30),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(
          'Discover',
          style: TextStyle(
            fontWeight: FontWeight.bold,
            fontSize: 30,
          ),
        ),
        SizedBox(
          height: 20,
        ),
        Row(
          children: [
            Expanded(
              child: TextField(
                cursorColor: Colors.black,
                decoration: InputDecoration(
                    filled: true,
                    fillColor: Colors.grey[200],
                    hintText: 'Search',
                    prefixIcon: Icon(Icons.search),
                    prefixIconColor: Colors.grey,
                    border: OutlineInputBorder(
                      borderSide: BorderSide(color: Colors.grey[300]!),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    enabledBorder: OutlineInputBorder(
                      borderSide: BorderSide.none,
                      borderRadius: BorderRadius.circular(10.0),
                    )),
              ),
            ),
            Container(
              margin: EdgeInsets.only(left: 10),
              width: 50,
              height: 50,
              decoration: BoxDecoration(
                color: mainColor,
                borderRadius: BorderRadius.circular(5),
              ),
              child: Icon(
                Icons.tune,
                color: Colors.white,
              ),
            )
          ],
        ),
      ]),
    );
  }
}

// class MountsList extends StatelessWidget {
//   const MountsList({super.key});

//   @override
//   Widget build(BuildContext context) {
//     MountModel cuurentMount = mountItems[1];
//     return GestureDetector(
//       child: Container(
//           alignment: Alignment.bottomLeft,
//           padding: const EdgeInsets.all(20),
//           margin: const EdgeInsets.all(10),
//           width: 150,
//           decoration: BoxDecoration(
//               borderRadius: BorderRadius.circular(15),
//               image: DecorationImage(
//                   image: NetworkImage(cuurentMount.path), fit: BoxFit.cover)),
//           child: Column(
//               mainAxisAlignment: MainAxisAlignment.end,
//               crossAxisAlignment: CrossAxisAlignment.start,
//               children: [
//                 Text(cuurentMount.name,
//                     style: const TextStyle(
//                         color: Colors.white, fontWeight: FontWeight.bold)),
//                 Text(cuurentMount.location,
//                     style: const TextStyle(color: Colors.white))
//               ])),
//       onTap: () {
//         print('${cuurentMount.description}');
//       },
//     );
//   }
// }

//helper=>دالة تعمل رتيرن للويدجت ب س البيرفورمنس كتير سيء/يستعملوها بدل ليست فيو بلدر
class Listviewbuilder extends StatelessWidget {
  const Listviewbuilder({super.key});

  @override
  Widget build(BuildContext context) {
    return ListView.builder(
        //مسؤول عن بناء الويدجت المكررة
        scrollDirection: Axis.horizontal,
        itemCount: mountItems.length,
        itemBuilder: (context, i) {
          // MountModel currentMount = mountItems[i];
          //return widget //context=>زي م احكيله انا وين
          return GestureDetector(
            child: Container(
                alignment: Alignment.bottomLeft,
                padding: const EdgeInsets.all(20),
                margin: const EdgeInsets.all(10),
                width: 150,
                decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(15),
                    image: DecorationImage(
                        image: NetworkImage(mountItems[i].path),
                        fit: BoxFit.cover)),
                child: Column(
                    mainAxisAlignment: MainAxisAlignment.end,
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(mountItems[i].name,
                          style: const TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.bold)),
                      Text(mountItems[i].location,
                          style: const TextStyle(color: Colors.white))
                    ])),
            onTap: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: ((context) => DetailsPage(mountItems[i]))));
              //print('${mountItems[i].description}');
            },
          );
        });
  }
}

class AppCategoryList extends StatelessWidget {
  const AppCategoryList({super.key});

  @override
  Widget build(BuildContext context) {
    //todo{2}
    return Column(children: [
      Container(
        padding: EdgeInsets.all(20),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(
              'Category',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),
            ),
            Text(
              'See More',
              style: TextStyle(
                  color: Colors.red, fontWeight: FontWeight.bold, fontSize: 15),
            ),
          ],
        ),
      ),
      Container(
        //مهم عشان اعطي الليست فيو الهايت
        height: 100,
        child: ListView.builder(
            // shrinkWrap: true,
            scrollDirection: Axis.horizontal,
            itemCount: categories.length,
            itemBuilder: ((context, index) {
              return Center(
                child: Container(
                  padding: EdgeInsets.all(27),
                  margin: EdgeInsets.only(left: 15, right: 15),
                  decoration: BoxDecoration(
                      border: Border.all(color: Colors.grey[300]!),
                      borderRadius: BorderRadius.circular(8)),
                  child: Column(children: [
                    Icon(
                      categories[index].icon,
                      color: mainColor,
                    ),
                    Text(
                      categories[index].category,
                      style: TextStyle(fontWeight: FontWeight.bold),
                    )
                  ]),
                ),
              );
            })),
      )
    ]);
  }
}

class DetailsPage extends StatelessWidget {
  //يلزمها داتا -الطريقة الابسط اخدها معي بكونستركتور الويدجت تاع البيج
  final MountModel mount;

  const DetailsPage(this.mount, {super.key});

  @override
  Widget build(BuildContext context) {
    var selectedItem = mount;

    return Scaffold(
        backgroundColor: Colors.white,
        body: Column(
          children: [
            Expanded(
                child: ClipRRect(
                    //تعمل شكل وهي من الماتيريل ويدجت //هو مش جايب كونتينر عشان عشان يعمل ريديس //ميزتها البوردر ريديس يعطيني بتون ليفت بوتون رايت
                    //الكارد والكليب خوات فيهم لمعان يعطوني لمعان خوات عوض عن الكونتينر
                    //الكارد بستخدمها لما احط داتا
                    borderRadius: const BorderRadius.only(
                        bottomLeft: Radius.circular(40),
                        bottomRight: Radius.circular(40)),
                    child: Stack(
                      children: [
                        Container(
                          padding: const EdgeInsets.all(30),
                          decoration: BoxDecoration(
                              image: DecorationImage(
                                  image: NetworkImage(selectedItem.path),
                                  fit: BoxFit.cover)),
                        ),
                        Positioned.fill(
                            //بدي احط في الستاك اشي بوزشن لكن بدي ياه يملي الستاك ، بحتاججها لما يكون عندي طبقة فوق مثلا ، دورها اكتر اشي فرونت ايند ، بستخدمها لما بدي احط داتا بيضا على الصورة ،طبقة تكون على الصورة خفيفة مش هتغطيها
                            child: Container(
                                //ياخد حجم ابنه ف الفيل بتخليه ياخد كله
                                decoration: BoxDecoration(
                                    gradient: LinearGradient(
                                        colors: [
                              Colors.transparent, //يبدا بهاد
                              Colors.black.withOpacity(0.7) //شفافية للاسود
                            ],
                                        begin: Alignment
                                            .center, //يبدأ//تاعت الستاك كله
                                        end: Alignment.bottomCenter)))),
                        Positioned(
                            bottom: 30,
                            left: 30,
                            right: 0,
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(selectedItem.name,
                                      style: const TextStyle(
                                          color: Colors.white,
                                          fontSize: 30,
                                          fontWeight: FontWeight.bold)),
                                  Text(selectedItem.location,
                                      style: const TextStyle(
                                          color: Colors.white, fontSize: 20)),
                                ])),
                        AppBar(
                            elevation: 0,
                            backgroundColor: Colors.transparent,
                            iconTheme: const IconThemeData(color: Colors.white),
                            title: const Center(
                                child: Icon(Icons.terrain,
                                    color: Colors.white, size: 40)),
                            actions: [
                              Container(
                                  margin: const EdgeInsets.only(right: 10),
                                  child: const Icon(Icons.pending,
                                      color: Colors.white, size: 30))
                            ])
                      ],
                    ))),
            Expanded(
                child: Column(
              children: [
                const DetailsRatingBar(),
                Expanded(
                    child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Padding(
                        padding: const EdgeInsets.only(
                            left: 20, right: 20, bottom: 20),
                        child: Text('About ${selectedItem.name}',
                            textAlign: TextAlign.left,
                            style: const TextStyle(
                                color: Colors.black,
                                fontSize: 20,
                                fontWeight: FontWeight.bold))),
                    Padding(
                        padding: const EdgeInsets.only(
                            left: 20, right: 20, bottom: 20),
                        child: Text(selectedItem.description,
                            style: const TextStyle(fontSize: 12)))
                  ],
                )),
                const DetailsBottomActions()
              ],
            ))
          ],
        ));
  }
}

class DetailsRatingBar extends StatelessWidget {
  const DetailsRatingBar({super.key});

  @override
  Widget build(BuildContext context) {
    var sampleRatingData = {'Rating': '4.6', 'Price': '\$12', 'Open': '24hrs'};

    return Container(
        padding: const EdgeInsets.all(10),
        child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: List.generate(
                sampleRatingData.entries.length,
                (index) => Container(
                    padding: const EdgeInsets.all(20),
                    margin: const EdgeInsets.all(10),
                    decoration: BoxDecoration(
                      border: Border.all(
                          color: Colors.grey.withOpacity(0.2), width: 2),
                      borderRadius: BorderRadius.circular(20),
                    ),
                    child: Column(children: [
                      Text(sampleRatingData.entries.elementAt(index).key,
                          style: const TextStyle(
                              color: Colors.black,
                              fontWeight: FontWeight.bold)),
                      Text(sampleRatingData.entries.elementAt(index).value,
                          style: const TextStyle(
                              color: Colors.red,
                              fontWeight: FontWeight.bold,
                              fontSize: 15))
                    ])))));
  }
}

class DetailsBottomActions extends StatelessWidget {
  const DetailsBottomActions({super.key});

  @override
  Widget build(BuildContext context) {
    return Padding(
        padding: const EdgeInsets.only(left: 20, right: 20, bottom: 20),
        child: Row(children: [
          Expanded(
              child: Material(
                  borderRadius: BorderRadius.circular(15),
                  color: mainColor,
                  child: InkWell(
                    highlightColor: Colors.white.withOpacity(0.2),
                    splashColor: Colors.white.withOpacity(0.2),
                    onTap: () {},
                    child: Container(
                        padding: const EdgeInsets.all(21),
                        child: const Text('Book Now',
                            textAlign: TextAlign.center,
                            style: TextStyle(color: Colors.white))),
                  ))),
          Container(
              margin: const EdgeInsets.only(left: 10),
              padding: const EdgeInsets.all(15),
              decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(15),
                  color: Colors.white,
                  border: Border.all(color: mainColor, width: 2)),
              child:
                  const Icon(Icons.turned_in_not, color: Colors.red, size: 25))
        ]));
  }
}


//الرجوع نافقيتور دوت بوب
